...
/* TODO Fix all limits. */
const I_DATE_LIMIT_WALL_GETCOMMENTS_GROUP = 60 * 60 * 24 * 1;
const I_DATE_LIMIT_WALL_GET_GROUP = 60 * 60 * 1;
const I_DATE_LIMIT_WALL_GET_USER = 60 * 60 * 24 * 26;
const I_NULL_VALUE = -1;
const I_VK_API_WALL_GETCOMMENTS_COUNT_DEFAULT = 100;
const I_VK_API_WALL_GET_COUNT_DEFAULT = 3;
...
function sms_db_posts_fetch_comments() {
  ...
  while ($a_i = $o_db_data_posts->fetchArray()) {
    ...
    if ($a_i['date'] < $i_timestamp - I_DATE_LIMIT_WALL_GETCOMMENTS_GROUP) {
      continue;
    }
    ...
    do {
      ...
      if ($o_result != null) {
            ...
            if ($o_ri['thread']['count'] > 0) {
              do {
                ...
                if ($o_result_nested != null) {
                  ...
                  sms_log('error, wall.getcomments, nested, https://vk.com/wall' . $i_db_owner_id . '_' . $i_db_post_id . '?reply=' . $i_db_comment_id);
                  break;
                }
        ...
        sms_log('error, wall.getcomments, https://vk.com/wall' . $a_i['owner_id'] . '_' . $a_i['post_id'] . '?reply=' . $a_i['comment_id']);
        break;
      }
  ...
  foreach ($a_watched_owners as $s_wo) {
    ...
    do {
      ...
      if ($o_result != null) {
        ...
        sms_log('error, wall.get, https://vk.com/wall-' . $s_wo . '?own=1&offset=' . $i_offset);
        break;
      }
